...
//	LCD_Color_Fill(x1,y1,x2,y2,color_p);
    bsp.lcd.blockwrite(x1,y1,x2,y2);
	...
	height=y2-y1+1;			//高度
 	for(i=0;i<height;i++)
	{
 		bsp.lcd.setxy(x1,y1+i);   	//设置光标位置 
		for(j=0;j<width;j++)
		{
            bsp.lcd.writedata(color_p->full);
//			Bsp_LCD->LCD_RAM =color_p->full;//写入数据
			color_p++;	
		}
	}	
	lv_flush_ready();
...
void tft_fill(int32_t x1, int32_t y1, int32_t x2, int32_t y2, lv_color_t color)
{
//		Bsp_LCD_ColorBox(x1,y1,x2,y2,color.full);		   				//填充单色
    bsp.lcd.colorblock(x1,y1,x2,y2,color.full);
}
